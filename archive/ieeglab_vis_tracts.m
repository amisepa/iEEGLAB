%% Visualize tracts
% NOT WORKING --> MISSING FILE: compact3T01_acq-diadem_space-T1w_desc-preproc_dwi.nii
% 
% ORIGINAL CODE: https://github.com/MultimodalNeuroimagingLab/HAPwave/blob/main/pcc_fig7a_dMRI_renderLimbic_diadem.m
% 
% Requirements:
%   - Along-tract (https://github.com/johncolby/along-tract-stats)
...
% antEls      = {'ROP1'};
% % set name of bundles
% all_trks    = {'Fornix_R', 'Cingulum_Parolfactory_R', 'Cingulum_Frontal_Parietal_R',...
%     'Cingulum_Parahippocampal_Parietal_R', 'Cingulum_Parahippocampal_R'};
...
antEls      = {'LK1'};
% set name of bundles
all_trks    = {'Fornix_L', 'Cingulum_Parolfactory_L', 'Cingulum_Frontal_Parietal_L',...
    'Cingulum_Parahippocampal_Parietal_L', 'Cingulum_Parahippocampal_L'};

sub_files = { dir(fullfile(EEG.filepath)).name }';

% load dMRI-brain
% diadem_file = sub_files(contains(sub_files, 'diadem_space-T1w_desc-preproc_dwi'))
% dwi_file    = fullfile(data_path,'derivatives','dMRI',['sub-' sub_label],'dwi',['sub-' sub_label '_ses-compact3T01_acq-diadem_space-T1w_desc-preproc_dwi.nii.gz']);
dwi_file = fullfile(EEG.filepath, sprintf('sub-%s_ses-mri01_T1w_deFaced.nii', sub_label));
ni_dwi      = niftiRead(dwi_file);
fg_fromtrk  = [];
for jj          = 1:length(all_trks)

    tkr_name        = all_trks{jj};
    trk_file = contains(sub_files, sprintf('%s.trk', tkr_name));

    if any(trk_file)
        disp("Track file found! Reading...")
        trk_file = sub_files{trk_file};
        [header,tracks] = trk_read(fullfile(EEG.filepath, trk_file));
        ...
        warning("Track file not found in subject folder.")
    end
end
...
loc_info    = readtable(EEG.ieeglab.opt.elec_tsv,'FileType','text','Delimiter','\t','TreatAsEmpty',{'N/A','n/a'});
...
% t1_name = sub_files{contains(sub_files, 'desc-preproc_T1w.nii')};
t1_name = dwi_file;
