...
mainDir   = '/Users/cedriccannard/Documents/MATLAB/eeglab/plugins/iEEGLAB';
...
bsl_method      = 'median';   % or 'median' (default), 'mean', 'trimmed_mean', or '1/f'
...
for pp = 1:nPairsRun
    ...
    stop_smp  = start_smp + nTime;

    % -------------------- Read MEFD data --------------------
    % [signal, kept] = read_mefd_data(mefd_folder, channel_names, start_smp, stop_smp);
    [~, signal] = ieeglab_load_mefd(mefd_folder, [], channel_names, 'samples', [start_smp stop_smp]);
    ...
    % sig2D = filter_ieeg(sig2D, fs, 'locutoff', 0.1, 'usefftfilt', true,  'plotfreqz', false);
    % sig2D = filter_ieeg(sig2D, fs, 'locutoff', 59, 'hicutoff', 61, 'revfilt', 1, ...
    %                     'usefftfilt', false, 'plotfreqz', false);
    % signal = reshape(sig2D, nChan, nTime, []);

    % -------------------- CAR (exclude stim + bad chans) --------------------
    if perform_CAR && ~isempty(good_chans_car)
        [signal, ~] = apply_ieeg_car(signal, timevec, good_chans_car, car_chans_ratio, car_time_int);
    end

    % -------------------- Baseline removal (uses bsl_method from top) --------------------
    if ndims(signal) == 2
        signal = reshape(signal, size(signal,1), size(signal,2), 1);
    end
    signal = rm_bsl_ieeg(signal, bsl_idx, bsl_method, fs);   % expects [chan x time x trials]

    % -------------------- two plots --------------------
    % Plot 1: heatmap of the average across trials (all channels x time)
    avg_pair = squeeze(trimmean(signal, 10, 3));             % [chan x time]
    plot_ccep(avg_pair, timevec, channel_names, 'all', [], 0.20);

    % Plot 2: single-channel overlay of all trials + mean
    chan_to_plot = min(118, nChan);                          % choose your channel
    ...
    for cc = 1:nElig
        ...
        % Keep trials x time for inspection
    ...
    % -------------------- Average across trials for this pair --------------------
    average_ccep(:, iPair, :) = avg_pair;   % [chan x time]
end
